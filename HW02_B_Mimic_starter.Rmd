...
ggplot(diamonds, aes(cut, fill = clarity)) +
  ...
  annotate("rect", xmin = 4.5, xmax = 5.5, ymin = 0, ymax = 5000, alpha = .3) +
...
iris$Species <- factor(iris$Species, levels = c("versicolor", "setosa","virginica"))
  ...
  geom_point() +
  scale_color_manual(values = c("#00BC59", "#FC717F", "#529EFF"))  +
  scale_shape_manual(values = c(17, 16, 15)) +
  facet_wrap(~Species, scales = "free") + scale_x_continuous(limits = c(4.3, 8)) +
  geom_smooth(method = lm, se = FALSE, color = "black", formula = y ~ x)
  
  #could't find the exact default color names, so I guesssed off of a color scale chart.
  iris$Species <- factor(iris$Species, levels = c("versicolor", "setosa","virginica"))
...
```{r graoh 3 code}
Not_corvette <- mpg[mpg$model != "corvette",]

ggplot(Not_corvette, aes(displ, hwy, label = paste("Corvette,", year))) + 
  geom_point() + 
...
#Couldn't make the labels lay out exactly as the picture did..
```
...
#couldn't get the jitter to line up just right
```
