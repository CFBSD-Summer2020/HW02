...
#can't figure out how to change the order of the facets.
...
ggplot(mpg)+
  ...
  geom_text_repel(data = subset(corvette), aes(displ, hwy, label = paste("Corvette,",year) ))+
  geom_point(data = subset(corvette), aes(displ, hwy), color ="blue")+
  labs(title = "Corvettes are a bit of an outlier")
#not sure why my labels aren't the exact same as the the original. 
```
...
ggplot(mpg, aes(cty, class, color = class))+
  ...
  geom_point(aes(fill = class), position = position_jitterdodge())+
  #coord_flip()+
...
#For some reason the general shape of my data looks the same but individual data points look different.
```