# What went wrong_Idongesit Ekpo
...
*Used __%>%__ instead of __+__. I added a __+__ to include the __geom_point__ function.
*Misassigned the x axis component as __city__ instead of __cty__. I labelled it correctly after looking at the data. 
*Misused __mapping()__. I deleted it and corrected the plot. 
...
*Solved. Define the colour in geom_point instead. 
```{r}
...
*Alpha changes the transparency of the points. Setting alpha to 2 on this plot makes it appear as a point on the plot. It appears as a second legend. You have to move the mapping aesthetic to the ggplot so that alpha is not considered as an aesthetic. 
*You can change the size of the points by defining their size under the geom_point function. 
...
*You may have to decrease the size of the legend so that it can fit into the c(5,40) position.
* To remove the legend title, use _legend.title= element_blank()_
* To remove the legend, you can set its position to "none"

```{r}
mpg %>% 
ggplot(mapping = aes(x = displ, y = hwy, color = class)) + 
  geom_point(alpha = 2) + 
  theme(legend.direction = "horizontal", legend.position = "none", legend.title= element_blank())
...
* You separated your points based on drv thereby creating different categories. You could combine the data so that you don't have three separate legends and categories by deleting __color = drv__
...
* Remove the colour categorization __color = manufacturer__ to get rid of the outline. Then in the geom_boxplot function define fill as the colour of your choice. 
...
* Change the axis text angle to 90 degrees.
