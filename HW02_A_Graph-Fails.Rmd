...
*R-* The parameter to change color was misplaced (should not be within mapping = aes())
```{r}
ggplot(data = mpg) + 
...
*R-* By modifying the alpha parameter you change the degree of transparency of the points. To modify the size you need to use the parameter size.
...
*R-* For the position, one should use the relative position in x and y (from 0 to 1, where 0 is the origin in both access and 1 the highest values). To remove the title I added theme(legend.title=element_blank()). 
...
*R-* Because the grouping based on "drv" (which is shown by color) is taking into account unless stated. Therefore, I added aes(color=NULL) within geom_smooth, so the color grouping was ignore when making the line. I also changed the line color to black.
```{r}
mpg %>% 
