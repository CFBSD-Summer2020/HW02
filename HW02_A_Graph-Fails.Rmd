...
`Answer:` It should be a + sign instead of a pipe before geom_point(), also the variable name is cty instead of city. 
```{r}
...
`Answer:`The coordinates on each axis actually ranges from 0 to 1, so we should use relative values instead of absolute values. We can remove the legend title by setting legnd.title to element_blank().
```{r}
mpg %>% 
ggplot() + 
...
`Answer:``"color = drv"` partitioned the data by drv, and geom_point() added this extra layer of scatter plot. 
```{r}
mpg %>% 
