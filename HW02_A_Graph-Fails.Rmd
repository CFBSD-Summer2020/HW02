...
mpg %>% 
  # the "%>%" needs to be replaced with a "+". Additionaly, the city data variable is abbreviated as "cty" in this dataset so the x value must be changed.
...
```{r}
ggplot(data = mpg) + 
  #the formatting needs to change slightly. The color variable needs to be assigned outside of the aes() brackets.
...
```{r}
# alpha changes the opacity of the plot's points. Instead, using the size argument will make the desired change.
mpg %>% 
ggplot() + 
  geom_point(mapping = aes(x = displ, y = hwy, color = class), size = 2) + 
  theme(legend.direction = "horizontal") + 
  #the scale for moving the legend is from 0 to 1. x = 0.5 and y = 0.9 approximately centers the legend at the top.
  theme(legend.position = c(.5, .9)) +
  #remove the legend title with element_blank()
...
```{r}
mpg %>% 
#add the "group" argument to aes() in order to have one smooth line.
ggplot(mapping = aes(x = displ, y = hwy, color = drv, group = 1)) + 
  geom_point() + 
...
```{r}
#use the "fill" argument instead of the "color" argument to fill the boxplots with color instead of just changing the outline. 
ggplot(data = mpg, mapping = aes(x = manufacturer, y = cty, fill = manufacturer)) + 
  geom_boxplot() + 
  #lower the x-axis labels so that they no longer overlap the plot with the "hjust" argument.
