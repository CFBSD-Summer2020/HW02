...
Legend.position at (0,0) represent bottom left and (1,1) represent top right and so, to move the legend on top empty space of the graph, we can assign (0.70, 0.90). The legend title can be removed by using element_blank() function. To remove the legend completely, we need to add theme(legend.position = none)
```{r}
mpg %>% 
ggplot() + 
...
ggplot(mapping = aes(x = displ, y = hwy), color = drv) + 
  geom_point() + 
  geom_smooth(se = F, color = "blue") #se = F makes it so it won't show the error in the line of fit
```
