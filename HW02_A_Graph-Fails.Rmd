...
```
**i got this Error: `mapping` must be created by `aes()` Did you use %>% instead of +?. This essentially tells us that
we have used pipe code `%>% `instead of operator `+`** *I also noticed that city =cty in the data, so cty must be used in the code*
##corrected code **graph 1**
```{r}
data(mpg) #this is a dataset from the ggplot2 package
...
```
#To change color of **geom_point()**
**I think the `color="blue"` argument ought to be outside of aes(). We may also move x and y variables to separate aes().**
##corrected code **graph 2**
```{r}
ggplot(data = mpg) + 
  geom_point(mapping = aes(x = displ, y = hwy), color = "blue")
# Alternate code could be
...
```
#To change size of geom_point
**Alpha sets the transparency of the points (0=transparent, 1=opaque).Thus `alpha=2` does not help. `size` argument is used to change the size of point in `geom_point()`**
## To change legend
legend can be put on top of graph by 'theme(legend.position="top")'. **legend.position** can be also a numeric vector c(x,y) and used to manually set the x and y coordinates of the legend box. values should be between **0** and **1** and in our case `c(0.6,0.9)` seems just fine to me. To **remove the legend title** call `element_blank()` within `theme()` layer. Finally to completely **hide the legend** for the **legendless plot** use `theme(legend.posiion="none")`.
##corrected code **graph 3**
```{r}
mpg %>% 
ggplot() + 
...
```
#Correcting **geom_smooth()**
Having `color=drv` inside `ggplot()` as `aes()` gives applies to all the three layers. Having `color=drv` **aes()** in `geom_point()` gives one smoothing line.
##correctd code **graph 4**
```{r}
mpg %>% 
...
```
##color fill **geom_boxplot**
** `color` arguments of `aes()` only changes the color of outlines while `fill` argument changes the color of the fill.Using `hjust=1` nicely aligns the x-axis labels.
##corrected code ** graph 5**
```{r}
