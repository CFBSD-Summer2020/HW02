...
*Because blue here is being used inside the* `aes()` *function, therefore, R is interpreting as an aesthetic of the graph (which is why appears in the legend). Instead, the color "blue" should be specified outside that function.*
...
*It is important to remember that in a numeric vector to specify the position of the legend* `c(x,y)`*, the values must be between 0-1, being* c(0,0) *bottom left corner, and* c(1,1) *top right corner. c(5,40) does not lead to any position within the graph*

*To remove the legend title, one must indicate the argument* `legend.title =` *and call* `element_blank()`*.*

*Lastly, to remove the legend altogether, you should write* `"none"` *in* `legend.position`*.*
...
*The problem here was that we were using drv as a color aesthetic, so ggplot was interpreting every color as a different set of data, not just one, because of that, several smooth lines were drawn. To correct it, you must quit drv from the* `aes()` *function.* 
