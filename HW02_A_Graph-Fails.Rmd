...
mpg %>%
  ...
  ggplot(mapping = aes(x = cty, y = hwy, color = "blue")) + #needed a + instead of a %>%, also city is just cty in the data frame.
  geom_point()
...
```{r}
ggplot(data = mpg) + 
  geom_point(mapping = aes(x = displ, y = hwy), color = "blue") #put "blue" out of aes
...
#Points can be made bigger with the size aesthetic. At 2 they're just a bit bigger.
  theme(legend.direction = "horizontal") +
  theme(legend.position = c(.6, .85)) +
#The ordered pair need to be within the panel space: top right c(1,1), bottom left c(0,0). They're proportions of the space, not references to data points.
  theme(legend.title = element_blank()) 
#Setting an element equal to element_blank() removes it.
...
#Placing the drv color aesthetic into the point geom stopped the geom_smooth from trying to add a line to three variables.
  geom_smooth()
```
...
#Fill changes the internal color, while color changes the outline.
  geom_boxplot() + 
  theme(axis.text.x = element_text(angle = 45, vjust = 0.7))
#vjust can change the vertical justification of text. I played around with values less than 1 until it looked nice.
```
