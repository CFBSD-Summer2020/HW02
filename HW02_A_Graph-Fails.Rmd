...
```{r}
head(mpg) #this is a dataset from the ggplot2 package
...
*Alpha is the transparency of the data points; setting it to 2 makes it more transparent*
*To make them bigger, we could change the size to 2*
...
*The coordinates of the legend box should be between 0 and 1*
```{r}
mpg %>% 
ggplot() + 
  geom_point(mapping = aes(x = displ, y = hwy, color = class), size = 2, show.legend = TRUE) + 
  theme(legend.direction = "horizontal")+
  theme(legend.position = c(5/8,40/47)) +
  theme(legend.title = element_blank())
```
*To remove the legend box, set show.legend to FALSE*
```{r}
mpg %>% 
ggplot() + 
  geom_point(mapping = aes(x = displ, y = hwy, color = class), size = 2, show.legend = FALSE)  
```
### Graph Fail 4
...
*This is because the data are grouped into 3 sets by drv, therefore it generates three lines for each set of data points*
```{r}
mpg %>% 
...
*Instead of assigning different color to each group based on certain category, we can directly set them to one color*
...
```{r}
ggplot(data = mpg, mapping = aes(x = manufacturer, y = cty, color = "red")) + 
